...
class Edge: #Stellt die Kanten im Netzwerk dar
	...
	def calc_weight(self): #später weiter präzisieren
		self.weight = round(self.c*self.n_cars + self.d, 4)#round(self.a*sigm(self.b*(self.n_cars-self.b)) + self.c*self.n_cars + self.d, 4)
...
class Net:#Stellt das Netzwerk dar
	def __init__(self, positions, data_a, data_b, data_c, data_d): #Initiert das Netzwerk #Eingabe als mehrere konstante Matrizen
		...
		for i in range(n_vertexes):
			connections = []
			for j, relation in enumerate(data_d[i]):
				if relation != 0:
